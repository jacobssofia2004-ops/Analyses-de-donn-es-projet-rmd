...
On choist un test de corrélation de Spearman car les données ne sont pas distribuées normalement (voir histogramme). Le coefficient de corrélation est de 0.32. C'est une corrélation positive mais la valuer reste faible car comprise entre 0.1 et 0.3 à plus ou moins 0.02, on a donc une faiblecorrélation entre la profondeur et la magnitude.
...
cor(log(df$Volume), df$Depth, method = "spearman")#pas de corrélation entre les deux variables)
```
Il 
...
Plusieurs remarques sont possibles. Tout d'abords, les magnitudes par volcans sont dépendantes, sinon l'ensebles des médiane serait alignées. De plus, médiane et moyenne ne sont pas égales, les magnitudes par volcans non donc pas une répatition modale. Enfin, les boxplots sont asymétique, on a donc une répartition non symétique de nos données.
Il est possible d'observer que certains groupes ont des distributions bimodal et non pas juste normal. On a pas de distribution normale.
...
anova_magmad <- aov(Depth ~ Classification, data = df)
hist(anova_magmad$residuals)
summary(anova_magmad)
```
...
# Analyse multivariée 
**Attention**
Pour faire des tests possible de faire ANCOVA pour quand deux variables quanti avec une variable quali 
...
test_chi1 <- chisq.test(tab1)
test_chi1#c'est pas mal 
```
```{r}
tab2 <- table (df$Volcanoes,df$Classification)
test_chi2 <- chisq.test(tab2)
test_chi2#c'est de la merde 
```
```{r}
tab3 <- table (df$VEI.approximatif,df$Volcanoes)
test_chi3 <- chisq.test(tab3)
test_chi3#c'est de la merde
```
## AFC
```{r}
...
**Attention**
demander à madame au prochain cours les petites questions que j'ai au sujet des tests 
