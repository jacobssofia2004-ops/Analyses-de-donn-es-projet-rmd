...
```
```{r}
#faire un log de la magnitude
model <- lm(Volume ~ Magnitude+ Depth, data = df)
...
rf_model <- randomForest(Magnitude ~ Depth + Volume , data = df,
...
chart.Correlation(df[,c("Depth","Magnitude","Volume")])
```
```{r}
