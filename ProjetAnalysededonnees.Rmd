...
### Profondeur en fonction du type de magma
```{r}
...
### Volume estimé en fonction des volcans
```{r}
...
```
test du chi 2 pas significatif car p-valeur > 0.05. Cela veut dire qu'il n'y a pas de relation significatif entre le VEI et les volcans 
...
## Equation logistique 
```{r}
df_logit <- df[, c("Depth", "Magnitude", "Volume", "Volcanoes")]
str(df_logit)
model <- multinom(Volcanoes ~ Depth + Magnitude + Volume, data = df_logit)
summary(model)

```
```{r}

mod_mult <- multinom(Mag.Type ~ Depth + Magnitude, data = df)
summary(mod_mult)

exp(coef(mod_mult))   # odds ratios
