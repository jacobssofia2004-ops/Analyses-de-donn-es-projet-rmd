...
```{r}
packages <- c("dplyr", "ggplot2", "tidyr","performance","car","maps","ggspatial","sf", "prettymapr","rnaturalearth","rnaturalearthdata","viridis")
...
ggplot(df_faible,aes(x=log(Volume)))+
  geom_histogram(binwidth = 0.2, fill="lightblue", color="black", alpha=0.7)+
  theme_minimal()+
  labs(title="Histogramme des volumes éruptés du groupes forts (>=3) ",
       x="Volumme érupté",
       y="")+
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=16))
```
