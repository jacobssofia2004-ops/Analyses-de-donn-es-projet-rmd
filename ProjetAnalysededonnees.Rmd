...
```{r}
packages <- c("dplyr", "ggplot2", "tidyr","performance","car","maps","ggspatial","sf", "prettymapr","rnaturalearth","rnaturalearthdata","viridis","see","patchwork","geosphere","reshape2","cowplot","factoextra","FactoMineR","plotly","PerformanceAnalytics","ade4","pheatmap")
...
pas relation linéaire entre les données donc pas possible de faire une étude anova par rapport à ça 

les données ne suivent pas une loi normal donc pas possible de faire une anova 
...
pour moi elles ne sont pas distibué normalement après cela se discute. Si on considère que cela est normal déjà on peut faire deux groupes et faire une anova à plusieurs variables 
```{r}
...
# ------------------------------------------------------
# 3. Densité avec couleurs semi-transparentes et contours fins
# ------------------------------------------------------
...
# ------------------------------------------------------
# 4. Combinaison propre et équilibrée
# ------------------------------------------------------
...
ACP 
```{r}
df_acp <- df[, c("Depth", "Magnitude", "Volume")]
chart.Correlation(df_acp)
```
```{r}
acp_sect <- dudi.pca(df_acp, scannf = FALSE, nf = ncol(df_acp))
str(acp_sect)
