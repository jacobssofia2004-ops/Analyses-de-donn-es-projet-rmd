...
```{r}
packages <- c("dplyr", "ggplot2", "tidyr","performance","car","maps","ggspatial","sf", "prettymapr")
...
ggplot() +
  annotation_map_tile(type = "osm", zoomin = -1) +  # Fond OpenStreetMap
  geom_point(
    data = df,
    aes(x = Longitude, y = Latitude, color = Volcanoes),
    size = 3, alpha = 0.9
  ) +
  coord_sf(xlim = c(12, 27), ylim = c(35, 41), expand = FALSE) +
  theme_minimal() +
  labs(
...
```
```{r}
ggplot(df_faible,aes(x=log(Volume)))+
  geom_histogram(binwidth = 0.2, fill="lightblue", color="black", alpha=0.7)+
  theme_minimal()+
  labs(title="Histogramme de la magnitude des éruptions fortes (>=3) ",
       x="Magnitude",
